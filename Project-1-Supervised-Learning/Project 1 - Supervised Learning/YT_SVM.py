...
plt.figure()
title = 'Attractiveness SVM(kernel = rbf): Performance x Training Set Size'
plt.title('\n'.join(wrap(title,60)))
...
plt.savefig('plots/Attractiveness_Analytics/SVM/Attractiveness_Analytics_rbf_PerformancexTrainingSetSize.png')
print 'plot complete'
### ---
...
for c in c_coefs:
    ...
    print 'plotting results'
    plt.figure()
    title = 'Attractiveness SVM(kernel = linear, with c = ' + str(c) +'): Performance x Training Set Size'
    plt.title('\n'.join(wrap(title,60)))
    plt.plot(offsets, test_err, '-', label='test error')
    plt.plot(offsets, train_err, '-', label='train error')
    plt.legend()
    plt.xlabel('Num Estimators')
    plt.ylabel('Mean Square Error')
    plt.savefig('plots/Attractiveness_Analytics/SVM/Attractiveness_Analytics_linear_' + str(c) + '_PerformancexTrainingSetSize.png')
    print 'plot complete'
    ### ---
...
for d in degrees:
    ...
    print 'plotting results'
    plt.figure()
    title = 'Attractiveness SVM(kernel = poly, with degree = ' + str(d) +'): Performance x Training Set Size'
    plt.title('\n'.join(wrap(title,60)))
    plt.plot(offsets, test_err, '-', label='test error')
    plt.plot(offsets, train_err, '-', label='train error')
    plt.legend()
    plt.xlabel('Num Estimators')
    plt.ylabel('Mean Square Error')
    plt.savefig('plots/Attractiveness_Analytics/SVM/Attractiveness_Analytics_poly_' + str(d) + '_PerformancexTrainingSetSize.png')
    print 'plot complete'
    ### ---
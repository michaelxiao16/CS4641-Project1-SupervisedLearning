...
if(not os.path.exists('plots/AP_Analytics/NN')):
    dirname = 'NN'
    os.mkdir('plots/AP_Analytics/%s' %dirname)
...
for activation in activation_functions:
    ...
    plt.figure()
    title = 'AP Analytics Neural Nets: Performance x Training Set Size using Activation ' + activation
    plt.title('\n'.join(wrap(title,60)))
    ...
    filename = 'APAnalytics_NN_' + activation + '_PerformancexTrainingSetSize.png'
    plt.savefig('plots/AP_Analytics/NN/' + filename)
    print 'plot complete'
    ### ---